...
Class Wdb_Service_Notice{
	...
	public function admin_notice_for_remote_service() {
		...
		$message = sprintf(
			wp_kses_post( '%1$s<p>%2$s</p>', 'saasprime-essential' ),
			$this->notice_title,
			$this->notice_content
		);
	...
	public function determined_display_area(){
		...
		if( 
			...
			$currentScreen && 
			$currentScreen->base === 'toplevel_page_wdb-saasprime-theme-parent' &&
			$currentScreen->id === 'toplevel_page_wdb-saasprime-theme-parent' && 
