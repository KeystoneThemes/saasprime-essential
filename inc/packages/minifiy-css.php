...
abstract class saasprime_MinificationSequenceFinder
{
    public $start_idx;
    public $end_idx;
    public $type;
...
class saasprime_StringSequenceFinder extends saasprime_MinificationSequenceFinder
{
...
class saasprime_QuoteSequenceFinder extends saasprime_MinificationSequenceFinder
{
...
$singlesaasprime_QuoteSequenceFinder = new saasprime_QuoteSequenceFinder('\'');
$doublesaasprime_QuoteSequenceFinder = new saasprime_QuoteSequenceFinder('"');
$blockCommentFinder = new saasprime_StringSequenceFinder('/*', '*/');
$minificationStore = array();

function saasprime_getNextMinificationPlaceholder(){
    global $minificationStore;
    return '<-!!-' . sizeof($minificationStore) . '-!!->';
}
function saasprime_getNextSpecialSequence($string, $sequences){
...
function saasprime_minify_CSS($css){
    global $minificationStore, $singlesaasprime_QuoteSequenceFinder, $doublesaasprime_QuoteSequenceFinder, $blockCommentFinder;
    $css_special_chars = array($blockCommentFinder, // CSS Comment
    $singlesaasprime_QuoteSequenceFinder, // single quote escape, e.g. :before{ content: '-';}
    $doublesaasprime_QuoteSequenceFinder); // double quote
    // pull out everything that needs to be pulled out and saved
    while ($sequence = saasprime_getNextSpecialSequence($css, $css_special_chars)){
        switch ($sequence->type){
        case '/*': // remove comments
        $css = substr($css, 0, $sequence->start_idx) . substr($css, $sequence->end_idx);
        break;
        default: // strings that need to be preservered
        $placeholder = saasprime_getNextMinificationPlaceholder();
